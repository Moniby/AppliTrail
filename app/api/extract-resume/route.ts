...
const instructions = `You extract CV information for AppliFlow.
...
export async function POST(request: Request) {
  ...
  try {
    response = await fetch("https://api.openai.com/v1/responses", {
      ...
      body: JSON.stringify({
        ...
        input: [
          ...
          { role: "user", content: [
            { type: "input_text", text: "Extract this uploaded CV into the editable AppliFlow Master CV fields." },
    ...
    return Response.json({ error: "AppliFlow could not reach the AI service. Please try again." }, { status: 502 });
  }
