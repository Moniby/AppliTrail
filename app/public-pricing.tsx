...
const plans = [
  {
    ...
    features: [
      ...
      [true, "Browser extension job importer"],
...
function termBadge(planId: "free" | "basic" | "standard", interval: typeof intervals[number]) {
  if (planId === "free" || interval.months === 1) return null;
  return `Save ${interval.id === "quarterly" ? "7" : interval.id === "six_month" ? "10" : "20"}%`;
}
  ...
  return <section id="pricing" className="landing-pricing">
    ...
    <div className="landing-pricing-grid pricing-comparison-grid">
      {plans.map((plan) => {
        ...
        const badge = termBadge(plan.id, interval);
        return <article className={`pricing-plan ${plan.id === "standard" ? "featured" : ""}`} key={plan.id}>
          <div className="pricing-plan-title"><span>{plan.name}</span>{badge && <em className="plan-term-badge">{badge}</em>}</div>
          ...
          <ul><li className="included"><span>✓</span>{plan.allowance} AI generations each month</li>{plan.features.map(([included, feature]) => <li className={included ? "included" : "not-included"} key={feature}><span aria-hidden="true">{included ? "✓" : "✕"}</span>{feature}</li>)}</ul>
        </article>;
      })}
    </div>
